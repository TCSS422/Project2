...
There is no one most efficient method among the schedulers we tested. Priority provides exceptionally good response for the highest-priority processes, but if the user wishes to run background compute processes, there is no way to ensure they get a good slice of system time. Lottery provides workable average-case behavior, but it will never be optimized (and in certain cases could be very bad, if an important process doesn't win the lottery for a long time). Round-robin is good with manual tweaking (which we've done with our producer/consumer pairs) but can likewise degenerate into bad behavior, and there's no way to bump a quick-response process up the chain when necessary. We believe that round-robin is probably the best choice for a simple OS, but for truly efficient and responsive operation, we would need to develop a more efficient and fairer priority scheduling algorithm.